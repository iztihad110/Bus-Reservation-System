...
void Bus :: install()
{
 ...
 void Bus::show_total_seats(string s)
 {
    ...
    for(int i=0; i< number_of_seats ; i++)
    {
        if(i%5==0)
                cout << endl;
        string temp;
        if(seatStatus[i]==s || seatStatus[i]=="Vacant") temp=seatStatus[i];
        else temp="Reserved";
        cout << "[" << i+1 << "]" << "[" << temp << "]"  << " ";
...
void Bus::reserveSeat(int n,string s)    ///Reserve n seats in bus
{
    if (reserved_seats+n <= number_of_seats )    ///Checks if bus is already not filled completely
        {
            ...
            for(int i=0; i<n; i++)
            {
                ...
                if(seatno > number_of_seats || seatno==0)
                {
                ...
                if( seatStatus[seatno-1] == "Vacant" )
                {
                    string userName;
                    //cout << "Enter user name: ";
                    //cin >> userName;
                    userName=s;
                    seatStatus[seatno-1] = userName;
                }
                else
                {
...
int Bus::get_available_seats(){
    return available_seats;
}
string Bus::getStartingPoint(){
